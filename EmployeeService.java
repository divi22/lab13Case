...
public List<String> getSalIncdetails(List<Employee>elist){
	List<String> e2=elist.stream().map(e->e.getFirstName()+" "+e.getLastName()+" "+e.getSalary()+" "+(e.getSalary()+e.getSalary()*0.15)).collect(Collectors.toList());
	return e2;
...
public List<Employee> sortByDept(List<Employee>elist) {
	...
	Comparator<Employee>deptId=(e1,e2)->e1.getDepartment().getDepartmentId().compareTo(e2.getDepartment().getDepartmentId());
	List<Employee> e6=elist.stream().filter(e->e.getDepartment()!=null).sorted(deptId).collect(Collectors.toList());
